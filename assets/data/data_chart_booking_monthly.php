...
header('Content-Type: application/json');
	...
	$res1 = pg_query("select d.date, sum(count(b.booking_id)) over() as count from (select to_char(date_trunc('day', (date '2017-01-01' + offs)), 'yyyy-mm-dd') as date from generate_series(0,30,1) as offs) d left outer join booking b on d.date=to_char(date_trunc('day', b.booking_from), 'yyyy-mm-dd') where product_name = 'HG' posix_getgroups(oid) by d.date order by d.date asc limit 1;");

	$res2 = pg_query("select d.date, sum(count(b.booking_id)) over() as count from (select to_char(date_trunc('day', (date '2017-02-01' + offs)), 'yyyy-mm-dd') as date from generate_series(0,27,1) as offs) d left outer join booking b on d.date=to_char(date_trunc('day', b.booking_from), 'yyyy-mm-dd') where product_name = 'HG' group by d.date order by d.date asc limit 1;");

	$res3 = pg_query("select d.date, sum(count(b.booking_id)) over() as count from (select to_char(date_trunc('day', (date '2017-03-01' + offs)), 'yyyy-mm-dd') as date from generate_series(0,30,1) as offs) d left outer join booking b on d.date=to_char(date_trunc('day', b.booking_from), 'yyyy-mm-dd') where product_name = 'HG' group by d.date order by d.date asc limit 1;");

	$res4 = pg_query("select d.date, sum(count(b.booking_id)) over() as count from (select to_char(date_trunc('day', (date '2017-04-01' + offs)), 'yyyy-mm-dd') as date from generate_series(0,29,1) as offs) d left outer join booking b on d.date=to_char(date_trunc('day', b.booking_from), 'yyyy-mm-dd') where product_name = 'HG' group by d.date order by d.date asc limit 1;");
